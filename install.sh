...
build_from_github()
{
    ...
    if [[ $REPO == secp256k1 || $REPO == libzmq || $REPO == gmp ]]; then
        CONFIGURATION=(${CONFIGURATION[@]/--prefix=*/--prefix=\/usr\/local})
        # display_message "$2 configure options"
    ...
    push_directory "$REPO"
    if [[ $REPO == gmp ]]; then
        make_current_directory_ $JOBS "${CONFIGURATION[@]}"
    else
...
build_from_github_()
{
    ...
    #    -DGMP_INCLUDE_DIR:PATH=/usr/local/include -DGMP_LIBRARIES_DIR:PATH=/user/local/lib
    #    -DSECP256K1_INCLUDE_DIR:PATH=$PREFIX/include -DSECP256K1_LIBRARIES_DIR:PATH=$PREFIX/lib \
    #    -DZEROMQ_INCLUDE_DIR:PATH=$PREFIX/include -DZEROMQ_LIBRARIES_DIR:PATH=$PREFIXlib
...
#   build_from_github espressocoder gmp master $PARALLEL ${GMP_OPTIONS[@]} "$@"
    build_from_github_ espressocoder metaverse master $PARALLEL ${METAVERSE_OPTIONS[@]} "$@"
}
